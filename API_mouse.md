...
## RX78::Device::ToRel<small>RX78设备.到绝对坐标</small>

将光标置于相对于坐标 (0, 0) 的位置 (x, y) 处，无移动轨迹。
...
## RX78::Device::ToAbs<small>RX78设备.到相对坐标</small>

将光标到置于当前坐标的位置 (x, y) 处，无移动轨迹。使用此方法前需要先[设置目标机器分辨率]()。
...
使用数学方程模拟移动轨迹。此方法为相对移动，使用前需要先[设置目标机器分辨率]()。
...
| sample<small>采样系数</small> | 此系数 * 到目标的直线距离 = 用几个点模拟轨迹，默认为 0.2     |
| sleep                         | 每个模拟点之间的延时，默认为 10                              |



## RX78::Device::Trace<small>RX78设备.沿轨迹移动</small>

根据传入的轨迹数据进行移动，此方法为相对移动，使用前需要先[设置目标机器分辨率]()。

| 参数  | 注释                                                         |
| ----- | ------------------------------------------------------------ |
| data  | 轨迹数据，易语言传递二维数组，VC++ 传递 vector&lt;RX78::Offset&gt;，如：<br>&nbsp;&nbsp;&nbsp;&nbsp;{ {0, 1}, {1, 3}, {2, 3} } |
| sleep | y 轴移动量                                                   |

## RX78::Device::Trace

重载版本。根据传入的轨迹数据进行移动，此方法为相对移动，使用前需要先[设置目标机器分辨率]()。

| 参数  | 注释                                                         |
| ----- | ------------------------------------------------------------ |
| data  | 轨迹数据，VC++ 传递 short\[\]\[2\]，如：<br>&nbsp;&nbsp;&nbsp;&nbsp;{ {0, 1}, {1, 3}, {2, 3} } |
